...
int main(int argc, char* argv[]){
		...
		while ( infile.getline (line, 100)){
			...
			char code[50];
	whitespace(line, charNum);
	while (line[charNum] != ' ' && line[charNum] != '\t' && continueLine && line[charNum] != 0){
		if (line[charNum] == '#')
			continueLine = false;
	...
	char* label[21] = {"LD\0", "LDi\0", "SD\0", "SDi", "ADD", "ADDi", "SUB", "SUBi", "MUL", "MULi", "DIV", "DIVi", "JMP", "JZ", "JNZ", "JGZ", "JGEZ", "JLZ", "JLEZ", "Code:", "Data:"};
	int i = 0;
	for (i = 0; i < 21; ++i){
		if (strcmp(code, label[i])== 0)
			break;
	}
	cout << i << endl;
	int dataCodeNum;
